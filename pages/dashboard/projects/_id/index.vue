    ...
    <section class="container mx-auto pt-8">
        ...
        <div class="block mb-2">
          <div
            ...
            :key="transaction.id"
          >
            <div class="w-full border border-gray-400 lg:border-gray-400 bg-white rounded p-8 flex flex-col justify-between leading-normal">
              <div>
                <div class="text-gray-900 font-bold text-xl mb-1">
                  {{ transaction.name }}
                </div>
                <p class="text-sm text-gray-600 flex items-center mb-2">
                  Rp. {{ new Intl.NumberFormat().format(transaction.amount) }} &middot; {{ new Date(transaction.created_at).toLocaleString() }}
                </p>
              </div>
            </div>
          </div>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    try {
      let campaign = await $axios.$get('/api/v1/campaigns/' + params.id)
      let transactions = await $axios.$get('/api/v1/campaigns/' + params.id + '/transactions')
  ...
  methods: {
    ...
    async upload() {
      ...
      formData.append('campaign_id', this.campaign.data.id)
